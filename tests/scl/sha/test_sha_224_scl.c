...
TEST_GROUP(test_sha_224_scl);

TEST_SETUP(test_sha_224_scl)
{
    (void) scl_init(&scl);
}

TEST_TEAR_DOWN(test_sha_224_scl)
{
}

TEST(test_sha_224_scl, msg_abc) {
    int32_t result = 0;
    sha_ctx_t ctx;
    ...
    result = scl_sha(SCL_HASH_SHA224, message,
...
TEST(test_sha_224_scl, msg_2_blocks) {
    int32_t result = 0;
    sha_ctx_t ctx;
