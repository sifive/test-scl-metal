...
TEST_SETUP(scl_test_sha_512) { (void)scl_init(&scl); }
...
TEST(scl_test_sha_512, msg_abc)
{
    ...
    result =
        scl_sha(SCL_HASH_SHA512, message, sizeof(message), digest, &digest_len);
...
TEST(scl_test_sha_512, msg_2_blocks)
{
    ...
    result = scl_sha(SCL_HASH_SHA512, message, sizeof(message) - 1, digest,
                     &digest_len);
    TEST_ASSERT_TRUE(0 == result);
    TEST_ASSERT_TRUE(SHA512_BYTE_HASHSIZE == digest_len);
    TEST_ASSERT_TRUE(0 ==
