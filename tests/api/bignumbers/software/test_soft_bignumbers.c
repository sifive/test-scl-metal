...
TEST(soft_bignumbers, bignum_memcpy_success)
{
    int result = 0;
    size_t word_size;
    uint64_t dest[24] = {0};
    ...
    scl_bignum_memcpy(dest, source, word_size);

    result = memcmp(dest, source, sizeof(source));
    TEST_ASSERT_TRUE(0 == result);
}

TEST(soft_bignumbers, bignum_memset_success) {
        int result = 0;
    size_t word_size;
    uint64_t array[24] = {
        0xF1A731C16826B112UL, 0xC2D6C36F322ADB31UL, 0xADF814D04293621AUL,
    ...
    scl_bignum_memset(array, 0xA5A5A5A5A5A5A5A5UL,word_size);

    result = memcmp(array, expected_array, sizeof(array));
    TEST_ASSERT_TRUE(0 == result);
}

TEST(soft_bignumbers, bignum_memcmp_a_equals_b)
{
    int32_t result = 0;
    size_t word_size;
    ...
    result = scl_bignum_memcmp(a, b, word_size);

    TEST_ASSERT_TRUE(0 == result);
}

TEST(soft_bignumbers, bignum_memcmp_a_greater_than_b_lsb)
{
    int32_t result = 0;
    size_t word_size;
...
TEST(soft_bignumbers, bignum_memcmp_a_greater_than_b_msb)
{
    int32_t result = 0;
    size_t word_size;
...
TEST(soft_bignumbers, bignum_memcmp_a_lower_than_b_lsb)
{
    int32_t result = 0;
    size_t word_size;
...
TEST(soft_bignumbers, bignum_memcmp_a_lower_than_b_msb)
{
    int32_t result = 0;
    size_t word_size;
